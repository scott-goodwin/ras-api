...
//      when(mockWsHttp.doGet(any())(any[HeaderCarrier], any[ExecutionContext])).thenReturn(Future.successful(HttpResponse(404)))
//      val result = await(TestConnector.getFileMetadata(envelopeId, fileId, userId))
//      result shouldBe None
//    }
//    "return None when file metadata return an exception" in {
//      when(mockWsHttp.doGet(any())(any[HeaderCarrier], any[ExecutionContext])).thenReturn(Future.failed(new RequestTimeoutException("")))
//      val result = await(TestConnector.getFileMetadata(envelopeId, fileId, userId))
//      result shouldBe None
//    }
  }
