...
class FileUploadConnectorSpec
  ...
  val fileMetadata: FileMetadata =
    FileMetadata(fileId, Some(originalFileName), Some(dateCreated))
  ...
  "getFile" should {
    ...
    val streamResponse: StreamedResponse = StreamedResponse(
      DefaultWSResponseHeaders(
        200,
        Map("CONTENT_TYPE" -> Seq("application/octet-stream"))),
      Source.apply[ByteString](
        Seq(ByteString("Test"), ByteString("\r\n"), ByteString("Passed"))
          .to[scala.collection.immutable.Iterable])
    )

    "return an StreamedResponse from File-Upload service" in {
      when(mockWsHttp.buildRequestWithStream(any())(any()))
        .thenReturn(Future.successful(streamResponse))

      val result = await(
        TestConnector.getFile(envelopeId, fileId, userId)(
          any[HeaderCarrier],
    ...
    "return a RuntimeException when File Upload throws error" in {
      ...
      exception.getMessage.contains(
        "Error Streaming file from file-upload service") shouldBe true
    }
  }
  ...
  "getFileMetadata" should {
    ...
    "return the original filename when file metadata returns 200" in {
      when(
        mockWsHttp.doGet(any[String], any[Seq[(String, String)]])(
          any[HeaderCarrier],
          any[ExecutionContext]))
        .thenReturn(
    ...
    "return None when file metadata does not return 200" in {
      when(
        mockWsHttp.doGet(any(), any[Seq[(String, String)]])(
          any[HeaderCarrier],
          any[ExecutionContext]))
  ...
  "deleteUploadedFile" should {
    ...
    "submit delete request to file-upload service" in {
      when(
        mockWsHttp.doDelete(any[String], any[Seq[(String, String)]])(
          any[HeaderCarrier],
          any[ExecutionContext]))
