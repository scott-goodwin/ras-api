...
class DesConnectorSpec
  ...
  object TestDesConnector extends DesConnector {
    ...
    override val error_InternalServerError: String =
      AppContext.internalServerErrorStatus
    ...
    override val error_ServiceUnavailable: String =
      AppContext.serviceUnavailableStatus
    override val error_TooManyRequests: String =
  ...
  val residencyStatusFailure =
    ResidencyStatusFailure("500", "HODS NOT AVAILABLE")
  ...
  "DESConnector getResidencyStatus with matching" should {
    ...
    "handle successful response when 200 is returned from des and CY and CYPlusOne is present" in {
      ...
      val successresponse = ResidencyStatusSuccess(
        ...
        deseasedIndicator = Some(false),
        currentYearResidencyStatus = "Uk",
        nextYearResidencyStatus = Some("Scottish")
      )

      val individualDetails = IndividualDetails("AB123456C",
                                                "JOHN",
                                                "SMITH",
                                                new DateTime("1990-02-21"))

      val expectedPayload = createJsonPayload(individualDetails)

      when(
        mockHttpPost.POST[JsValue, HttpResponse](any(), any(), any())(any(),
                                                                      any(),
                                                                      any(),
                                                                      any()))
        .thenReturn(Future.successful(
          HttpResponse(200, Some(Json.toJson(successresponse)))))

      val result = await(
        TestDesConnector.getResidencyStatus(individualDetails, userId, V2_0))
      result.isLeft shouldBe true
      result.left.get shouldBe ResidencyStatus("otherUKResident",
                                               Some("scotResident"))

      verify(mockHttpPost, times(1)).POST[JsValue, HttpResponse](
        any(),
        Meq(expectedPayload),
        any())(any(), any(), any(), any())
    }
    ...
    "When the requested API version is V1.0" should {
      ...
      "handle successful response when 200 is returned from des where CY and CYPlusOne is present for a Welsh resident" in {
        ...
        val successResponse = ResidencyStatusSuccess(
          ...
          deathDateStatus = Some(""),
          deseasedIndicator = Some(false),
          currentYearResidencyStatus = "Welsh",
          nextYearResidencyStatus = Some("Welsh")
        )

        val individualDetails = IndividualDetails("AB123456C",
                                                  "JACK",
                                                  "OSCAR",
                                                  new DateTime("1962-07-07"))

        val expectedPayload = createJsonPayload(individualDetails)

        when(
          mockHttpPost.POST[JsValue, HttpResponse](
            any(),
            Meq[JsValue](expectedPayload),
            any())(any(), any(), any(), any())).thenReturn(Future.successful(
          HttpResponse(200, Some(Json.toJson(successResponse)))))

        val result = await(
          TestDesConnector.getResidencyStatus(individualDetails, userId, V1_0))
      ...
      "handle successful response when 200 is returned from des where CY and CYPlusOne is present for a Scottish resident moving in Wales" in {
        ...
        val individualDetails = IndividualDetails("SG123480",
                                                  "HELEN",
                                                  "SMITH",
                                                  new DateTime("1975-02-18"))
        ...
        val result = await(
          TestDesConnector.getResidencyStatus(individualDetails, userId, V1_0))
        result shouldBe Left(
          ResidencyStatus("otherUKResident", Some("scotResident")))
    ...
    "When the requested API version is V2.0" should {
      ...
      "handle successful response when 200 is returned from des where CY and CYPlusOne is present for a Welsh resident" in {
        ...
        val result = await(
          TestDesConnector.getResidencyStatus(individualDetails, userId, V2_0))
        result shouldBe Left(
          ResidencyStatus("welshResident", Some("welshResident")))
      ...
      "handle successful response when 200 is returned from des where CY and CYPlusOne is present for a Scottish resident moving in Wales" in {
        ...
        val result = await(
          TestDesConnector.getResidencyStatus(individualDetails, userId, V2_0))
        result shouldBe Left(
          ResidencyStatus("welshResident", Some("scotResident")))
    ...
    "handle successful response when 200 is returned from des and only CY is present and feature toggle is turned on" in {
      ...
      val successresponse =
        ResidencyStatusSuccess(nino = "AB123456C",
                               deathDate = Some(""),
                               deathDateStatus = Some(""),
                               deseasedIndicator = None,
                               currentYearResidencyStatus = "Uk",
                               nextYearResidencyStatus = None)
    ...
    "handle unsuccessful response for bulk request when 429 is returned from des the first time around and CY and " +
      ...
      object TestDesConnector extends DesConnector {
        ...
        override val error_InternalServerError: String =
          AppContext.internalServerErrorStatus
        override val error_Deceased: String = AppContext.deceasedStatus
        override val error_MatchingFailed: String =
          AppContext.matchingFailedStatus
        override val error_DoNotReProcess: String =
          AppContext.doNotReProcessStatus
        override val error_ServiceUnavailable: String =
          AppContext.serviceUnavailableStatus
        override val error_TooManyRequests: String =
          AppContext.tooManyRequestsStatus
      ...
      val errorResponse = ResidencyStatusFailure("INTERNAL_SERVER_ERROR",
      ...
      when(
        mockHttpPost.POST[JsValue, HttpResponse](any(), any(), any())(any(),
                                                                      ...
                                                                      any()))
        .thenReturn(Future.successful(
                      HttpResponse(429, Some(Json.toJson(errorResponse)))),
                    Future.successful(
                      HttpResponse(200, Some(Json.toJson(successresponse)))))

      val result = await(
        TestDesConnector.getResidencyStatus(individualDetails,
                                            userId,
                                            V2_0,
                                            isBulkRequest = true))
      ...
      verify(mockHttpPost, times(2)).POST[JsValue, HttpResponse](
        any(),
        Meq[JsValue](expectedPayload),
        any())(any(), any(), any(), any())
    }
    ...
    "handle unsuccessful response for bulk request when 429 is returned from des the first time around and CY and " +
      ...
      when(
        mockHttpPost.POST[JsValue, HttpResponse](any(), any(), any())(
          any(),
          any(),
          any(),
          any())).thenReturn(
        Future.successful(HttpResponse(429, Some(Json.toJson(errorResponse)))))
    ...
    "handle successful response when 200 is returned from des and only CY is present and no next year status toggle is turned off" in {
      ...
      val errorResponse =
        ResidencyStatusFailure(TestDesConnector.error_InternalServerError,
                               "Internal server error.")
    ...
    "handle failure response (no match) from des" in {
      ...
      when(
        mockHttpPost.POST[JsValue, HttpResponse](any(), any(), any())(
          ...
          any(),
          any())).thenReturn(
        Future.successful(HttpResponse(404, Some(Json.toJson(errorResponse)))))
    ...
    "handle success response but the person is deceased from des" in {
      val successResponse = ResidencyStatusSuccess(
        nino = "AB123456C",
        deathDate = Some("2017-12-25"),
        deathDateStatus = None,
        deseasedIndicator = Some(true),
        currentYearResidencyStatus = "Uk",
        nextYearResidencyStatus = Some("Uk")
      )
      ...
      val expectedResult = ResidencyStatusFailure(
        "DECEASED",
        "Cannot provide a residency status for this pension scheme member.")
    ...
    "handle unexpected responses as 500 from des" in {
      ...
      when(
        mockHttpPost.POST[JsValue, HttpResponse](any(), any(), any())(
          ...
          any(),
          any())).thenReturn(
        Future.successful(HttpResponse(500, Some(Json.toJson(errorResponse)))))
      ...
      verify(mockHttpPost, times(3)).POST[JsValue, HttpResponse](
        any(),
        Meq[JsValue](expectedPayload),
        any())(any(), any(), any(), any())
    }
    ...
    "handle Service Unavailable (503) response from des" in {
      implicit val formatF = ResidencyStatusFormats.failureFormats
      val errorResponse =
        ResidencyStatusFailure("SERVICE_UNAVAILABLE", "Service unavailable")
      ...
      when(
        mockHttpPost.POST[JsValue, HttpResponse](any(), any(), any())(any(),
                                                                      ...
                                                                      any()))
        .thenReturn(
          Future.failed(Upstream5xxResponse("SERVICE_UNAVAILABLE", 503, 503)))
    ...
    "handle bad request from des" in {
      ...
      val expectedErrorResponse =
        ResidencyStatusFailure("INTERNAL_SERVER_ERROR",
                               "Internal server error.")
      val errorResponse =
        ResidencyStatusFailure("DO_NOT_RE_PROCESS", "Internal server error.")
      ...
      when(
        mockHttpPost.POST[JsValue, HttpResponse](any(), any(), any())(
          ...
          any(),
          any())).thenReturn(
        Future.successful(HttpResponse(400, Some(Json.toJson(errorResponse)))))
    ...
    "Handle requests" when {
      ...
      "429 (Too Many Requests) has been returned 3 times" in {
        ...
        val errorResponse = ResidencyStatusFailure("TOO_MANY_REQUESTS",
