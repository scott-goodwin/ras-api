...
class FileControllerSpec
  ...
  val fileController = new FileController {
    ...
    override def getFile(name: String,
                         userId: String): Future[Option[FileData]] =
      Some(fileData)
    override def deleteFile(name: String,
                            id: String,
                            userId: String): Future[Boolean] = true
  ...
  when(mockRasFileRepository.removeFile(any(), any(), any()))
  ...
  "FileController" should {
    "serve a file" when {
      "valid filename is provided" in {
        ...
        val result = await(
          fileController
            .serveFile("testFile.csv")
            .apply(FakeRequest(Helpers.GET, "/ras-api/file/getFile/:testFile")))
    ...
    "give NOT_FOUND response" when {
      val fileController = new FileController {
        ...
        override def getFile(name: String,
                             userId: String): Future[Option[FileData]] = None
    ...
    "remove a file" when {
      "already saved fileName is provided" in {
        ...
        when(
          mockAuthConnector.authorise[Enrolments](any(), any())(any(), any()))
          .thenReturn(successfulRetrieval)
        when(mockRasChunksRepository.removeChunk(any()))
          .thenReturn(Future.successful(true))
        val fileName = "testFile.csv"
        val result = await(
          fileController
            .remove(fileName, "5b46289f71f77f7bb48eda45")
            .apply(FakeRequest(Helpers.DELETE,
                               s"/ras-api/file/remove/:${fileName}")))
        result.header.status shouldBe Status.OK
        verify(mockAuditService).audit(
          auditType = Meq("FileDeletion"),
          path = any(),
          auditData = Meq(
            Map("userIdentifier" -> "A123456",
      ...
      "chunks deletion fails as id cannot be converted to a BSONObjectId" in {
        val fileController = new FileController {
          ...
          override def parseStringIdToBSONObjectId(
              id: String): Try[BSONObjectID] = Failure(new Throwable("Failure"))
        }
