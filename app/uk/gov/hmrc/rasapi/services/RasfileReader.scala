...
trait RasFileReader {
  ...
  def readFile(envelopeId: String, fileId: String, userId: String)(
    ...
    fileUploadConnector.getFile(envelopeId, fileId, userId).map {
      ...
      case None =>
        Logger.error(
          s"File Processing: problem reading data in the file for userId ($userId).")
...
trait RasFileWriter {
  ...
  def createFileWriter(fileId: String, userId: String): FILE_INFO = {
    ...
    file.isSuccess match {
      ...
      case false =>
        Logger.error(
          s"Error creating temp file for writing results for userId ($userId).")
        throw new FileNotFoundException
    }
  }

  def writeResultToFile(writer: BufferedWriter,
                        line: String,
    ...
    try {
      ...
      case ex: Throwable =>
        Logger.error(
          s"Exception in writing line to the results file ${ex.getMessage} for userId ($userId).")
        throw new RuntimeException(
          s"Exception in writing line to the results file ${ex.getMessage}")
    }
    true
  }
  ...
  def closeWriter(writer: BufferedWriter): Boolean = {
    ...
    res.recover {
      case ex: Throwable =>
        Logger.error(
          s"Failed to close the Outputstream with error ${ex.getMessage}.")
        false
    }
    true
  }
  ...
  def clearFile(path: Path, userId: String): Unit =
    if (!Files.deleteIfExists(path))
      Logger.error(
        s"error deleting file or file ${path} doesn't exist for userId ($userId).")
}
