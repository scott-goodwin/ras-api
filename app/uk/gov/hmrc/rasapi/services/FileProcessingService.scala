...
object FileProcessingService extends FileProcessingService {
  ...
  override val INTERNAL_SERVER_ERROR: String =
    AppContext.internalServerErrorStatus
  override val SERVICE_UNAVAILABLE: String = AppContext.serviceUnavailableStatus
  override val FILE_PROCESSING_MATCHING_FAILED: String =
    AppContext.fileProcessingMatchingFailedStatus
  override val FILE_PROCESSING_INTERNAL_SERVER_ERROR: String =
    AppContext.fileProcessingInternalServerErrorStatus
}

trait FileProcessingService
    extends RasFileReader
    with RasFileWriter
    with ResultsGenerator {
  ...
  def manipulateFile(inputFileData: Try[Iterator[String]],
    ...
    try {
      ...
      writeResultToFile(
        writer._2,
        s"National Insurance number,First name,Last name,Date of birth,$getTaxYearHeadings",
        userId)
      dataIterator.foreach(row =>
        if (!row.isEmpty) {
          writeResultToFile(writer._2,
                            fetchResult(removeDoubleQuotes(row),
                                        userId,
                                        callbackData.fileId,
                                        apiVersion = apiVersion),
                            userId)
      ...
      Logger.warn(
        s"File results complete, ready to save the file (fileId: ${callbackData.fileId}) for userId ($userId).")
      ...
      case ex: Throwable => {
        Logger.error(
          s"error for userId ($userId) in File processing -> ${ex.getMessage}")
        sessionCacheService.updateFileSession(
          userId,
          callbackData.copy(status = STATUS_ERROR),
          None,
          None)
        fileResultsMetrics.stop
      }
  ...
  def saveFile(filePath: Path, userId: String, callbackData: CallbackData)(
      implicit hc: HeaderCarrier,
      request: Request[AnyContent]): Unit = {
      ...
      fileRepo
        ...
        .onComplete { result =>
          result match {
            case Success(file) =>
              Logger.warn(
                s"Starting to save the file (${file.id}) for user ID: $userId")

              val resultsFileMetaData = Some(
                ResultsFileMetaData(file.id.toString,
                                    file.filename,
                                    file.uploadDate,
                                    file.chunkSize,
                                    file.length))
              ...
              fileUploadConnector
                .getFileMetadata(callbackData.envelopeId,
                                 callbackData.fileId,
                                 userId)
                .onComplete {
                  ...
                  case Failure(ex) =>
                    Logger.error(
                      s"Failed to get File Metadata for file (${file.id}), for user ID: $userId.",
                      ex)
                    sessionCacheService.updateFileSession(userId,
                                                          callbackData,
                                                          resultsFileMetaData,
              ...
              Logger.warn(
                s"Completed saving the file (${file.id}) for user ID: $userId")

            case Failure(ex) => {
              Logger.error(
                s"results file for userId ($userId) generation/saving failed with Exception ${ex.getMessage}")
              sessionCacheService.updateFileSession(
                userId,
                callbackData.copy(status = STATUS_ERROR),
                None,
  ...
  def getTaxYearHeadings = {
    ...
    if (currentDate.isAfter(new DateTime(currentYear - 1, 12, 31, 0, 0, 0, 0)) && currentDate
          .isBefore(new DateTime(currentYear, 4, 6, 0, 0, 0, 0)))
