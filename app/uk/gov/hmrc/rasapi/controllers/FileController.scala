...
import uk.gov.hmrc.rasapi.repository.{
  RasChunksRepository,
  RasFileRepository,
  RasRepository
}
...
trait FileController extends BaseController with AuthorisedFunctions {
  ...
  def parseStringIdToBSONObjectId(id: String): Try[BSONObjectID] =
        ...
        getFile(fileName, id)
          .map { fileData =>
            if (fileData.isDefined) {
              Logger.debug(
                "[FileController] [serverFile] File repo enumerator received")
              val byteArray = Source.fromPublisher(
                Streams.enumeratorToPublisher(
                  fileData.get.data.map(ByteString.fromArray)))
              apiMetrics.stop()
              Ok.sendEntity(
                  HttpEntity.Streamed(byteArray,
                                      Some(fileData.get.length),
                                      Some(_contentType)))
                .withHeaders(
              ...
              Logger.error(
                s"[FileController] [serverFile] Requested File not found to serve fileName is $fileName")
          ...
          .recover {
            case ex: Throwable =>
              Logger.error(
                s"[FileController] [serverFile] Request failed with Exception ${ex.getMessage} for userId ($id) file -> $fileName")
              InternalServerError
          }
      } recoverWith {
        handleAuthFailure
  ...
  def remove(fileName: String, fileId: String): Action[AnyContent] =
    Action.async { implicit request =>
      ...
      authorised(
        AuthProviders(GovernmentGateway) and (Enrolment(PSA_ENROLMENT) or Enrolment(
          PP_ENROLMENT))).retrieve(authorisedEnrolments) { enrols =>
        val id = getEnrolmentIdentifier(enrols)
        deleteFile(fileName, fileId: String, id)
          .flatMap { res =>
              ...
              case Success(bsonId) =>
                chunksRepo
                  ...
                  .map { isChunkRemoved =>
                    if (isChunkRemoved) {
                      Logger.warn(
                        s"[FileController][remove] Chunk deletion succeeded, fileId is: ${fileId}")
                      auditService.audit(
                      ...
                      Logger.error(
                        s"[FileController][remove] Chunk deletion failed, fileId is: ${fileId}")
                    }
                  }
                  .recover {
                    case ex: Throwable => {
                      Logger.error(
                        s"Caught exception: ${ex.getMessage} ${ex.printStackTrace}")
                    }
                  }
                  .map(_ => ())
              case Failure(ex) => {
                Logger.error(
                  s"[FileController][remove] The following fileId ($fileId) could not be converted to a BSONObjectId.")
                auditService.audit(
                  auditType = "FileDeletion",
                  path = request.path,
                  auditData = Map(
          ...
          .recover {
            case ex: Throwable =>
              Logger.error(
                s"[FileController][remove] Request failed with Exception ${ex.getMessage} for userId ($id) file -> $fileName")
              InternalServerError
          }
      } recoverWith {
        handleAuthFailure
  ...
  private def handleAuthFailure(implicit request: Request[_])
    : PartialFunction[Throwable, Future[Result]] =
    PartialFunction[Throwable, Future[Result]] {
      case ex: InsufficientEnrolments =>
        Logger.warn(
          "[FileController] [handleAuthFailure] Insufficient privileges")
        Metrics.registry.counter(UNAUTHORIZED.toString)

        Future.successful(Unauthorized(toJson(Unauthorised)))

      case ex: NoActiveSession =>
        Logger.warn("[FileController] [handleAuthFailure] Inactive session")
        Metrics.registry.counter(UNAUTHORIZED.toString)
        Future.successful(Unauthorized(toJson(InvalidCredentials)))
      case e =>
        Logger.warn(
          s"[FileController] [handleAuthFailure] Internal Error ${e.getCause}")
  ...
  def deleteFile(name: String,
    ...
    fileRepo.removeFile(name, fileId, userId)
  // $COVERAGE-ON$
