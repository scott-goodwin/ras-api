...
trait LookupController
  ...
  implicit class VersionUtil(request: Request[_]) {
    def getVersion: ApiVersion = {
      request.headers
        ...
        .getOrElse(throw new BadRequestException(
          Json.toJson(ErrorAcceptHeaderInvalid).toString))
      // the exception in the else should never be thrown since it wouldn't pass validation otherwise
    }
  }

  def getResidencyStatus(): Action[AnyContent] =
    validateAccept(acceptHeaderValidationRules).async { implicit request =>
      val apiMetrics = Metrics.responseTimer.time
      authorised(
        AuthProviders(GovernmentGateway) and (Enrolment(PSA_ENROLMENT) or Enrolment(
          PP_ENROLMENT))).retrieve(authorisedEnrolments) { enrols =>
        ...
        withValidJson(
          ...
          (individualDetails) => {
            desConnector
              .getResidencyStatus(individualDetails, id, request.getVersion)
              .map {
                case Left(residencyStatusResponse) =>
                  val residencyStatus =
                    if (residencyYearResolver.isBetweenJanAndApril())
                      ...
                      residencyStatusResponse.copy(
                        nextYearForecastResidencyStatus = None)
                  auditResponse(failureReason = None,
                                nino = Some(individualDetails.nino),
                                residencyStatus = Some(residencyStatus),
                                userId = id)
                  Logger.debug(
                    s"[LookupController][getResidencyStatus] Residency status returned successfully for userId ($id).")
                  apiMetrics.stop()
                  ...
                  matchingFailed.code match {
                    case STATUS_DECEASED =>
                      auditResponse(failureReason = Some(STATUS_DECEASED),
                                    ...
                                    userId = id)
                      Logger.debug(
                        s"[LookupController][getResidencyStatus] Individual is deceased for userId ($id).")
                    ...
                    case STATUS_MATCHING_FAILED =>
                      auditResponse(failureReason = Some("MATCHING_FAILED"),
                                    ...
                                    userId = id)
                      Logger.debug(
                        s"[LookupController][getResidencyStatus] Individual not matched for userId ($id).")
                      Metrics.registry.counter(FORBIDDEN.toString)
                      Forbidden(toJson(IndividualNotFound))
                    case STATUS_TOO_MANY_REQUESTS =>
                      auditResponse(failureReason =
                                      Some(STATUS_TOO_MANY_REQUESTS),
                                    nino = Some(individualDetails.nino),
                                    residencyStatus = None,
                                    userId = id)
                      Logger.error(
                        s"[LookupController][getResidencyStatus] Too Many Requests for userId ($id).")
                    ...
                    case STATUS_SERVICE_UNAVAILABLE =>
                      auditResponse(failureReason = Some("SERVICE_UNAVAILABLE"),
                                    ...
                                    userId = id)
                      Logger.debug(
                        s"[LookupController][getResidencyStatus] Service unavailable for userId ($id).")
                    ...
                    case _ =>
                      auditResponse(failureReason =
                                    ...
                                    userId = id)
                      Logger.error(
                        s"[LookupController][getResidencyStatus] Error returned from DES, error code: " +
              ...
              case th: Throwable =>
                auditResponse(failureReason =
                              ...
                              userId = id)
                Logger.error(
                  s"[LookupController][getResidencyStatus] Error occurred for userId ($id), " +
                    s"Exception message: ${th.getMessage}",
                  th)
                Metrics.registry.counter(INTERNAL_SERVER_ERROR.toString)
                InternalServerError(toJson(ErrorInternalServerError))
            }
          ...
          errors => {
            Logger.info(
              s"The errors for userId ($id) are: " + errors.toString())
            Future.successful(BadRequest(
              toJson(ErrorBadRequestResponse(errorConverter.convert(errors)))))
          }
        )
      } recoverWith {
        ...
        case e =>
          Logger.warn(s"Internal Error ${e.getCause}")
          Future.successful(
            InternalServerError(toJson(ErrorInternalServerError)))
      }
  ...
  private def withValidJson(
    ...
    request.body.asJson match {
      case Some(json) =>
        Try(
          json.validate[IndividualDetails](
            IndividualDetails.individualDetailsReads)) match {
          case Success(JsSuccess(payload, _)) => {
            ...
            Try(onSuccess(payload)) match {
              ...
              case Failure(ex: Exception) =>
                Logger.error(
                  s"[LookupController] An error occurred in Json payload validation for userId ($userId) ${ex.getMessage}")
                Future.successful(
                  InternalServerError(toJson(ErrorInternalServerError)))
            }
          }
          ...
          case Failure(e) =>
            Logger.error(
              s"[LookupController] An error occurred due to ${e.getMessage} returning " +
  ...
  private def auditResponse(failureReason: Option[String],
    ...
    val ninoMap: Map[String, String] =
      nino.map(nino => Map("nino" -> nino)).getOrElse(Map())
...
object LookupController extends LookupController {
  ...
  override val STATUS_TOO_MANY_REQUESTS: String =
    AppContext.tooManyRequestsStatus
  override val STATUS_SERVICE_UNAVAILABLE: String =
