...
import uk.gov.hmrc.play.microservice.filters.{
  AuditFilter,
  LoggingFilter,
...
trait DataCleansing extends GlobalSettings with RunMode {
  ...
  override def onStart(app: Application): Unit = {
    ...
    AppContext.removeChunksDataExerciseEnabled match {
      case true => {
        Logger.info(
          "[data-cleansing-exercise] [on-start] Starting data exercise for removing of chunks")
        DataCleansingService.removeOrphanedChunks()
      }
      case false =>
        Logger.warn(
          "[data-cleansing-exercise] [on-start] No data exercise carried")
  ...
  override def onError(request: RequestHeader,
    ...
    super.onError(request, ex) map (res => {
      res.header.status match {
        case 401 =>
          Status(Unauthorised.httpStatusCode)(Json.toJson(Unauthorised))
        case _ =>
          Status(ErrorInternalServerError.httpStatusCode)(
            toJson(ErrorInternalServerError))
      }
    })
  }
}

object RasShortLivedHttpCaching
  ...
  override lazy val domain = getConfString(
    ...
    throw new Exception(
      s"Could not find config 'cachable.short-lived-cache.domain'"))
...
object RasShortLivedCache extends ShortLivedCache {
  override implicit lazy val crypto = new ApplicationCrypto(
